...
- 默认模式：保持系统蓝牙状态不变；桌面小部件会等待最多 3 秒后开始解锁。
- Shizuku 模式：支持独立 Shizuku 应用或 Sui 提供的 Shizuku API，需要启动服务并授权。解锁前若蓝牙未开启则自动开启，解锁流程结束后（无论成功、失败或超时）自动关闭由应用打开的蓝牙；若蓝牙是用户手动开启的，则全程保持开启不变。主界面和桌面小部件使用相同流程。
- Android 11 及以上可以通过无线调试启动 Shizuku；无 Root 启动方式在设备重启后需要重新确认 Shizuku 服务已运行。
